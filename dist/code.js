...
function runSync(payload) {
  return __async(this, null, function* () {
    ...
    yield buildSystemLibraryPage(designSystem);
    for (const flowSummary of pds.product.flows) {
      ...
      yield buildFlowPage(flowDefinition, designSystem);
    }
  });
}
function buildSystemLibraryPage(designSystem) {
  return __async(this, null, function* () {
    var _a, _b, _c;
    ...
    for (const category of sortedCategories) {
      ...
      for (const [logicalId, entry] of categoryEntries.sort(
        ...
        const node = yield createComponentInstance(entry.figma_ref, logicalId, entry.category);
...
function buildFlowPage(flow, designSystem) {
  return __async(this, null, function* () {
    var _a;
    ...
    for (const breakpoint of breakpoints) {
      ...
      for (const screen of screensForBreakpoint) {
        ...
        for (const componentDef of components) {
          const node = yield createNodeForComponentInstance(
            componentDef,
            designSystem
          );
    ...
    yield wireInteractions(flow.screens, screenNodeMap, componentNodeMap);
    yield buildPartsArea(page, flow, Array.from(uniqueComponentIds), designSystem, board);
    page.selection = [board];
  });
}
function buildPartsArea(page, flow, componentIds, designSystem, board) {
  return __async(this, null, function* () {
    ...
    for (const componentId of componentIds.sort()) {
      ...
      const node = entry ? yield createComponentInstance(entry.figma_ref, componentId, entry.category) : yield createPlaceholderNode(`Missing mapping for ${componentId}`);
...
function wireInteractions(screens, screenNodeMap, componentNodeMap) {
  return __async(this, null, function* () {
    var _a, _b;
...
function createNodeForComponentInstance(componentDef, designSystem) {
  return __async(this, null, function* () {
...
function createComponentInstance(figmaRef, logicalId, category) {
  return __async(this, null, function* () {
...
function createPlaceholderNode(message) {
  return __async(this, null, function* () {
