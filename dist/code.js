...
const FONT_NAME = { family: 'Inter', style: 'Regular' };
...
async function buildFlowPage(flow, designSystem) {
  ...
  for (const breakpoint of breakpoints) {
    ...
    for (const screen of screensForBreakpoint) {
      ...
      const instancesForScreen = new Map();
...
async function createComponentInstance(
  ...
  if (componentNode.type === 'COMPONENT') {
    ...
    const baseVariant = defaultVariant ?? componentNode.children[0];
    if (!baseVariant) {
