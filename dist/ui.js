(function () {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    window.onmessage = (event) => {
      ...
      if (message.type === 'sync-success') {
        setStatus(
          statusEl,
          `Sync complete. Generated ${message.details.flows} flow page(s).`,
        );
        button.disabled = false;
      }
  ...
  function buildContentsUrl(details, path) {
    const encodedPath = path
      ...
      .join('/');
    return `https://api.github.com/repos/${details.owner}/${details.repo}/contents/${encodedPath}?ref=${encodeURIComponent(
      details.branch,
    )}`;
  }
